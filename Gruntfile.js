...
module.exports = function (grunt) {
	...
	const plugin_arg = grunt.option('plugins');
	const custom_file = path.resolve(
		process.cwd(),
	...
	if (plugin_arg) {
		const plugin_args = plugin_arg.split(/\s*,\s*/);

		plugin_args.forEach((plugin_name) => {
			custom_content.push(
				`import ${plugin_name} from './plugins/${plugin_name}/plugin.js';`
			);
		});
		plugin_args.forEach((plugin_name) => {
			custom_content.push(
				`TomSelect.define('${plugin_name}', ${plugin_name});`
			);
		});
	...
	const scss_plugin_files = [];
	const matched_files = grunt.file.expand(['src/plugins/*/plugin.scss']);
	for (let i = 0, n = matched_files.length; i < n; i++) {
		const plugin_name = matched_files[i].match(/src\/plugins\/(.+?)\//)[1];
	...
	const scss_plugin_path_replace_options = {
		patterns: [
			{
	...
	grunt.initConfig({
		...
		copy: {
			scss: {
				files: [
					{
						'build/scss/tom-select.scss': [
							'src/scss/tom-select.scss',
						],
						'build/scss/tom-select.default.scss': [
							'src/scss/tom-select.default.scss',
						],
						'build/scss/tom-select.bootstrap4.scss': [
							'src/scss/tom-select.bootstrap4.scss',
						],
						'build/scss/tom-select.bootstrap5.scss': [
		...
		replace: {
			...
			builddocs: {
				...
				files: [
					{
						src: ['build-docs/js/index.bundle.js'],
						dest: 'build-docs/js/index.bundle.js',
					},
					{
						src: ['build-docs/index.html'],
				...
				files: [
					{
						expand: true,
						flatten: false,
						src: ['build/scss/tom-select.scss'],
						dest: '',
					},
				],
			},
		...
		sass: {
			...
			build: {
				files: [
					{
						'build/css/tom-select.css': [
							'src/scss/tom-select.scss',
						],
						'build/css/tom-select.default.css': [
							'src/scss/tom-select.default.scss',
						],
						'build/css/tom-select.bootstrap4.css': [
							'src/scss/-tom-select.bootstrap4.scss',
						],
						'build/css/tom-select.bootstrap5.css': [
							'src/scss/-tom-select.bootstrap5.scss',
						],
					},
				],
		...
		postcss: {
			...
			min: {
				...
				files: [
					{
						'build/css/tom-select.min.css': [
							'build/css/tom-select.css',
						],
						'build/css/tom-select.default.min.css': [
							'build/css/tom-select.default.css',
						],
						'build/css/tom-select.bootstrap4.min.css': [
							'build/css/tom-select.bootstrap4.css',
						],
						'build/css/tom-select.bootstrap5.min.css': [
							'build/css/tom-select.bootstrap5.css',
						],
					},
				],
		...
		watch: {
			...
			docs: {
				files: ['doc_src/**'],
				tasks: ['builddocs', 'check_doc_links'],
			},
