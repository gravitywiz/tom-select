describe('duplicates', function () {
	it_n('should add and remove duplicates', async function () {
		...
		assert.equal(
			...
			1,
			'should have 1 option w/ value=a in original select'
		);
		...
		assert.equal(
			test.instance.input.querySelectorAll('option[value="a"]').length,
			3,
			'should have 3 option w/ value=a in original select'
		);
	...
	it_n('should initialize with duplicates (input)', async () => {
		const test = setup_test('<input value="a,a">', { duplicates: true });
		assert.equal(test.instance.items.length, 2);
	});

	it_n('should initialize with duplicates (select)', async () => {
		const test = setup_test(
			'<select multiple><option selected>a</option><option selected>a</option></select>',
			{ duplicates: true }
		);
		assert.equal(test.instance.items.length, 2);
	});
