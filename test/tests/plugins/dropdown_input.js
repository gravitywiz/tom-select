describe('plugin: dropdown_input', function () {
	it_n('dropdown should open onclick', async () => {
		const test = setup_test(
			'<input value="a,b" tabindex="1" placeholder="test placeholder" />',
			{ plugins: ['dropdown_input'] }
		);
		const placeholder =
			test.instance.control.querySelector('.items-placeholder');

		assert.isFalse(
			isVisible(placeholder),
			'items-placeholder should not be visible'
		);
		assert.isTrue(
			test.instance.dropdown.contains(test.instance.control_input),
			'control_input should be in dropdown'
		);
	...
	it_n('dropdown should open onclick without available options', async () => {
		const test = setup_test(
			'<select multiple><option selected>A</option></select>',
			{ plugins: ['dropdown_input'] }
		);
		await asyncClick(test.instance.control);
		assert.equal(test.instance.isOpen, true);
	...
	it_n('should select option with [enter] keypress (single)', async () => {
		const test = setup_test('AB_Single', { plugins: ['dropdown_input'] });
	...
	it_n(
		'should load results for "a" after loading results for "ab"',
		function (done) {
			const expected_load_queries = ['ab', 'a'];

			const test = setup_test('AB_Single', {
				plugins: ['dropdown_input'],
				load(query, load_cb) {
					const expected_load_query = expected_load_queries.shift();
	...
	it_n('[esc] to close & [down] to open', async () => {
		...
		assert.isTrue(
			isVisible(placeholder),
			'items-placeholder should be visible'
		);
