describe('plugin: virtual_scroll', function () {
	...
	it_n('load more data while scrolling', async () => {
		...
		const test = setup_test('<input>', {
			...
			load(query, callback) {
				load_calls++;
				const url_params = this.getUrl(query);
				const data = DataProvider(url_params[0], url_params[1]);
		...
		assert.equal(
			...
			1,
			'should have loading-more-reuslts template'
		);
		assert.equal(
			test.instance.dropdown_content.querySelectorAll('.no-more-results')
				.length,
			0,
			'should not have no-more-results template'
		);
		assert.equal(
			test.instance.dropdown_content.querySelectorAll('.option').length,
			21,
			'Should display 20 options plus .loading-more-results'
		);
		...
		assert.equal(
			Object.keys(test.instance.options).length,
			40,
			'should load second set of data'
		);
		assert.equal(
			test.instance.dropdown_content.querySelectorAll(
				'.loading-more-results'
			...
			0,
			'should not have loading-more-reuslts template'
		);
		assert.equal(
			test.instance.dropdown_content.querySelectorAll('.no-more-results')
				.length,
			1,
			'should have no-more-results template'
		);
		assert.equal(
			test.instance.dropdown_content.querySelectorAll('.option').length,
			31,
			'Should display 30 options plus .no-more-results'
		);
		...
		assert.equal(
			Object.keys(test.instance.options).length,
			20,
			'should load new set of data for "b"'
		);
	...
	it_n('keyboard navigation', async () => {
		...
		const loading_more = test.instance.dropdown_content.querySelector(
			'.loading-more-results'
		);
		assert.equal(
			Object.keys(test.instance.options).length,
		...
		assert.equal(
			index,
			index,
			'active option should be first of newest options'
		);
		assert.equal(
			Object.keys(test.instance.options).length,
			40,
