describe('ESM Module', function () {
	...
	it_n('isKeyDown', async () => {
		...
		await waitFor(100);
		assert.equal(
			util_module.isKeyDown('shiftKey', last_keydown),
			false,
			'should return false if [alt] is pressed'
		);
		...
		assert.equal(
			util_module.isKeyDown('shiftKey', last_keydown),
			false,
			'should return false if [alt][shift] is pressed'
		);
		...
		assert.equal(
			util_module.isKeyDown('shiftKey', last_keydown),
			false,
			'should return false if [' + shortcut_key + '][shift] is pressed'
		);
		assert.equal(
			util_module.isKeyDown('ctrlKey', last_keydown),
			false,
			'should return false if [' + shortcut_key + '][shift] is pressed'
		);
		...
		assert.equal(
			util_module.isKeyDown('shiftKey', last_keydown),
			true,
			'should return true if [shift] is pressed'
		);
		...
		await waitFor(100);
		assert.equal(
			util_module.isKeyDown(shortcut_key + 'Key', last_keydown),
			true,
			'should return true if [' + shortcut_key + '] is pressed'
		);
