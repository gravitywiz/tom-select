...
describe('Interaction', function () {
	describe('dropdown', function () {
		it_n(
			'should keep dropdown open after selection made if closeAfterSelect: false',
			function (done) {
				const test = setup_test('AB_Multi', {});

				click(test.instance.control, function () {
					click(
						$('[data-value=a]', test.instance.dropdown_content),
						function () {
		...
		it_n(
			'should keep dropdown open when clicking on dropdown if dropdownParent=body',
			function (done) {
				const test = setup_test('AB_Multi', { dropdownParent: 'body' });
		...
		it_n(
			'should reopen dropdown if clicked after being closed by closeAfterSelect: true',
			function (done) {
				const test = setup_test('AB_Multi', { closeAfterSelect: true });
				...
				click(test.instance.control, function () {
					click(
						...
						function () {
							...
							assert.equal(
								test.instance.isFocused,
								true,
								'should be closed w/ focus after select'
							);
		...
		it_n(
			'should close dropdown after creating item if closeAfterSelect: true',
			async () => {
				const evts = [];
				...
				const create_option =
					test.instance.dropdown.querySelector('.create');
		...
		it_n(
			'should close dropdown after selection made if closeAfterSelect: true and in single mode',
			async () => {
				const evts = [];

				const test = setup_test('AB_Single', {
					closeAfterSelect: true,
				});
		...
		it_n(
			'should not close dropdown after selection made if closeAfterSelect: false and in single mode',
			async () => {
				const test = setup_test('AB_Single', {
		...
		it_n(
			'should close dropdown and clear active items after [escape] key press',
			async () => {
				...
				assert.equal(
					test.instance.control_input.value,
					'',
					'should clear control_input'
				);
			}
		);

		it_n(
			'should close dropdown and clear control_input after [escape] key press',
			async () => {
				const test = setup_test('AB_Multi', { items: ['a'] });
		...
		it_n(
			'should change activeOption with [down] and [up] keypress',
			async () => {
				...
				await asyncClick(test.instance.control);
				assert.equal(
					test.instance.activeOption.dataset.value,
					'a',
					'activeOption should be "a" to start'
				);
				...
				assert.equal(
					test.instance.activeOption.dataset.value,
					'b',
					'activeOption should be changed to "b"'
				);
				...
				assert.equal(
					test.instance.activeOption.dataset.value,
					'a',
					'activeOption should be changed back to "a"'
				);
			}
		);

		it_n(
			'should not close dropdown when clicking on dropdown header',
			function (done) {
				const select = `<select><optgroup label="Swedish Cars">
				...
				click(test.instance.control, function () {
					...
					const header =
						test.instance.dropdown.querySelector(
							'.optgroup-header'
						);
	...
	describe('clicking control', function () {
		it_n('should toggle focus', function (done) {
			const test = setup_test('AB_Single_Long', {
				items: ['p'],
			});
			...
			click(test.instance.control, function () {
				...
				assert.equal(
					test.instance.activeOption.dataset.value,
					'p',
					'activeOption should be "p"'
				);
				assert.isAbove(
					test.instance.dropdown_content.scrollTop,
					0,
					'dropdown should be scrolled to activeOption'
				);
		...
		it_n(
			'should toggle focus clicking item in single mode control',
			function (done) {
				const test = setup_test('AB_Single', {
					items: ['a'],
				});
		...
		it_n(
			'should remain open but clear active item on click',
			function (done) {
				const test = setup_test('AB_Multi');
	...
	describe('clicking label', function () {
		it_n('should give focus to select', function (done) {
			const test =
				setup_test(`<label for="labeledSelect" id="select-label">select</label><select class="setup-here" id="labeledSelect">
		...
		it_n('should give focus to input', function (done) {
			const test =
				setup_test(`<label for="labeledInput" id="input-label">input</label>
	...
	describe('selecting option', function () {
		it_n('should select option when clicked', async () => {
			const test = setup_test(
				'<div id="test-wrap"><select class="setup-here">' +
			...
			await asyncClick(test.instance.control);
			assert.equal(
				test.instance.input.querySelectorAll('option').length,
				3,
			...
			assert.equal(
				option_before,
				option_after,
				'should not recreate original <option>'
			);
			assert.equal(
				test.instance.input.value,
				'b',
				'should select "b" value'
			);
			assert.equal(
				test.instance.dropdown_content.querySelectorAll('.selected')
					.length,
				1,
				'only one dropdown option should have selected class'
			);
			assert.isFalse(clicked, 'should not trigger click evt on div');
			assert.isFalse(test.instance.isOpen, 'Should close dropdown');
			assert.isFalse(
				isVisible(test.instance.dropdown),
		...
		it_n('should update original select when required', async () => {
			const test = setup_test(
				`<select required><option value="a" selected>A</option><option value="b">B</option></select>`
			);
		...
		it_n('should order selected options', function (done) {
			const test = setup_test('AB_Multi', { create: true });

			click(test.instance.control, function () {
				const option_a = test.html.querySelector('option[value="a"]');
				const option_b = test.html.querySelector('option[value="b"]');
				assert.equal(
					option_a.nextSibling,
					option_b,
					'should be original order'
				);

				click(
					$('[data-value="b"]', test.instance.dropdown),
					function () {
						syn.type(
							'new',
									...
									function () {
										click(
											...
											function () {
												...
												assert.equal(
													option_b.nextSibling.value,
													'new',
													'"new" should be after "b"'
												);
												assert.equal(
													option_b.nextSibling
														.nextSibling,
													option_a,
													'"a" should be after "b"'
												);
		...
		it_n(
			'should remove user created option when selecting other option',
			async () => {
				const test = setup_test('AB_Single', {
					create: true,
					persist: false,
				...
				const len_opts_before = Object.keys(
					test.instance.options
				).length;
				test.instance.createItem('test');
				expect(Object.keys(test.instance.options).length).to.be.equal(
					len_opts_before + 1
				);
	...
	describe('typing in input', function () {
		it_n('should filter results', function (done) {
			...
			click(test.instance.control, function () {
				syn.type('a', test.instance.control_input).delay(
					0,
					function () {
						assert.equal(
							test.instance.control_input.value,
							'a',
							'control_input should have value="a"'
						);
						expect(
							$('[data-value="a"]', test.instance.dropdown).length
						).to.be.equal(1);
						expect(
							$('[data-value="b"]', test.instance.dropdown).length
						).to.be.equal(0);
						...
						click(test.instance.control_input, function () {
							assert.equal(
								test.instance.control_input.value,
								'a',
								'should not clear control_input'
							);
							done();
						});
					}
		...
		it_n('should refreshOptions when deleting filter', function (done) {
			...
			click(test.instance.control, function () {
				syn.type('a', test.instance.control_input).delay(
					...
					function () {
						assert.equal(
							test.instance.dropdown_content.children.length,
							1
						);

						syn.type('\b', test.instance.control_input).delay(
							0,
							function () {
								assert.equal(
									test.instance.dropdown_content.children
										.length,
									3
								);
								done();
		...
		it_n(
			'should hide dropdown if no results present and no_result template is null',
			async () => {
				const test = setup_test('AB_Multi', {
		...
		it_n(
			'should show no_results message if no results present',
			function (done) {
				const test = setup_test('AB_Multi');

				click(test.instance.control, function () {
					syn.type('awaw', test.instance.control_input).delay(
						0,
						function () {
							expect(test.instance.isOpen).to.be.equal(true);
							expect(
								test.instance.dropdown.children.length
							).to.be.equal(1);
							expect(
		...
		it_n(
			'should not hide dropdown if "create" option enabled and no results present',
			function (done) {
				const test = setup_test('AB_Multi', { create: true });
		...
		it_n(
			'should restore dropdown visibility when backing out of a query without results (backspace)',
			function (done) {
				const test = setup_test('AB_Multi', {});

				click(test.instance.control, function () {
					syn.type('awf', test.instance.control_input).type(
						'\b\b\b',
						test.instance.control_input,
						function () {
							expect(test.instance.isOpen).to.be.equal(true);
		...
		it_n(
			'should not move caret when [left] or [right] pressed and control_input is not empty',
			function (done) {
		...
		it_n(
			'should not create input if comma entered in single select mode',
			function (done) {
				const test = setup_test(
					'<select>' +
						'<option value="">Select an option...</option>' +
		...
		it_n(
			'should not delete any dropdown option text if duplicate match occurs',
			function (done) {
				const test = setup_test(
					'<select>' +
						'<option></option>' +
						...
						function () {
							expect(
								$(test.instance.dropdown_content)
									.find('.option[data-value=b]')
									.text()
							).to.be.equal('Isabel Street');
							done();
						}
					);
				});
	...
	describe('selecting items', function () {
		it_n(
			'should select previous item when [' +
				shortcut_key +
				'][left] pressed',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should select next item when [' +
				shortcut_key +
				'][right] pressed, then unselect when [' +
				shortcut_key +
				'][left] pressed',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should select item before control when [' +
				shortcut_key +
				'][left] pressed',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should not select next item when [' +
				shortcut_key +
				'][right] pressed at the end of item list',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		it_n('clicking item should activate it', async () => {
			...
			test.instance.addItem('a');
			const item = test.instance.getItem('a');
		...
		it_n(
			'should select option with [enter] keypress (single)',
			async () => {
				const test = setup_test('AB_Single');
		...
		it_n(
			'should select option with [enter] keypress (multiple)',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should not select option with [enter] keypress when dropdown is empty',
			async () => {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should select option with [tab] keypress when selectOnTab = true',
			function (done) {
				const test = setup_test('AB_Single', { selectOnTab: true });
		...
		it_n(
			'should not select option with [tab] keypress when selectOnTab = false (default)',
			function (done) {
				const test = setup_test(`<div>
		...
		it_n(
			'should select all items when [' + shortcut_key + '-a] pressed',
			async () => {
				const test = setup_test('AB_Multi');
				...
				await asyncType(
					'[' + shortcut_key + ']a[' + shortcut_key + '-up]'
				);
		...
		it_n(
			'should select text when control_input is not empty and [' +
				shortcut_key +
				'-a] pressed',
			async () => {
				const test = setup_test('AB_Multi');
		...
		it_n(
			'should not close dropdown when [' +
				shortcut_key +
				'-a] pressed no items',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		const ShiftMousedown = function (first_item, last_item) {
			it_n(
		...
		it_n(
			'should toggle active item when [' + shortcut_key + '] + mousedown',
			function (done) {
				const test = setup_test('AB_Multi');
		...
		const DeleteActiveItem = function (keypress) {
			it_n(
				'should remove active item when delete pressed',
				function (done) {
					const test = setup_test('AB_Multi');
		...
		it_n('should remove item when backspace pressed', function (done) {
			...
			assert.equal(
				Array.from(test.instance.input.options).filter(
					(option) => option.selected
				).length,
				2,
				'option.selected.length should = 2'
			);
			...
			click(test.instance.control, function () {
				syn.type('\b', test.instance.control_input, function () {
					...
					assert.equal(
						option_b,
						option_after,
						'should not remove original <option>'
					);
					assert.equal(
						Array.from(test.instance.input.options).filter(
							(option) => option.selected
						...
						1,
						'option.selected.length should = 1'
					);
					...
					syn.type('\b', test.instance.control_input, function () {
						...
						assert.equal(
							...
							0,
							'option.selected.length should = 0'
						);
		...
		it_n(
			'should prevent typing when there are active items',
			function (done) {
				const test = setup_test('AB_Multi');
				test.instance.addItem('a');
		...
		it_n(
			'should create item on [enter] when option_create template is null',
			function (done) {
				const test = setup_test('AB_Single', {
					create: true,
					render: {
		...
		it_n(
			'should not create item on [tab] when option_create template is null',
			function (done) {
				const test = setup_test('AB_Multi', {
					create: true,
					render: {
	...
	describe('blurring the input', function () {
		it_n(
			'should close dropdown when createOnBlur is true',
			function (done) {
				const test = setup_test('AB_Multi', {
					createOnBlur: true,
					create(value) {
				...
				click(test.instance.control, function () {
					syn.type('fooo', test.instance.control_input).delay(
						0,
						function () {
							expect(test.instance.isOpen).to.be.equal(true);
							expect(
							...
							syn.click(document.body).delay(5, function () {
								expect(test.instance.isOpen).to.be.equal(false);
								expect(
									isVisible(test.instance.dropdown)
								).to.be.equal(false);
	...
	describe('creating items', function () {
		it_n(
			'should create item when clicking on create option',
			async function () {
				const select =
					'<select multiple><option value="aa">aa</option><option value="ab">ab</option><option value="ba">ba</option></select>';
				const test = setup_test(select, { create: true });
				...
				assert.isFalse(
					test.instance.activeOption.classList.contains('create'),
					'create item should not be focused when addPrecedence=false (default)'
				);
		...
		it_n(
			'create item should be focused when addPrecedence=true',
			async function () {
				const select =
					'<select multiple><option value="aa">aa</option><option value="ab">ab</option><option value="ac">ac</option></select>';
				const test = setup_test(select, {
					create: true,
					addPrecedence: true,
				});
		...
		it_n(
			'create item should be focused when addPrecedence=false (default)',
			function (done) {
				const test = setup_test('AB_Multi', { create: true });
				...
				click(test.instance.control, function () {
					syn.type('b', test.instance.control_input, function () {
						assert.equal(
							test.instance.activeOption.classList.contains(
								'create'
							),
							false
						);
		...
		it_n(
			'should not show create option for existing options',
			async function () {
				const test = setup_test('AB_Multi', { create: true });
				...
				assert.isNotOk(
					create_option,
					'should not have create option for "a"'
				);
			}
		);

		it_n(
			'should show create option for existing options if duplicates=true',
			async function () {
				const test = setup_test('AB_Multi', {
					create: true,
					duplicates: true,
		...
		it_n('should focus create option with [up] keypress', function (done) {
			...
			click(test.instance.control, function () {
				assert.equal(
					test.instance.activeOption.dataset.value,
					'aa',
					'activeOption should be "aa" to start'
				);
				...
				syn.type('a', test.instance.control_input, function () {
					assert.equal(
						test.instance.activeOption.dataset.value,
						'aa',
						'activeOption should still be "aa"'
					);
					...
					syn.type('[up]', test.instance.control_input, function () {
						assert.equal(
							...
							true,
							'activeOption should be create option'
						);
		...
		describe('filtering created items', function () {
			...
			function execFilterTest(filter, done, expectation) {
				...
				click(instance.control, function () {
					syn.type(text, instance.control_input)
						.type(
							instance.settings.delimiter,
							instance.control_input
						)
						.delay(0, function () {
			...
			execFilterTests(
				'should add an item if the input matches the createFilter',
				[
					'a',
					/a/,
					function () {
			...
			execFilterTests(
				'should not add an item or display the create label if the input does not match the createFilter (B)',
				[
					'foo',
					/foo/,
					function () {
				...
				function (instance) {
					expect(
						$(instance.dropdown_content).filter('.create').length
					).to.be.equal(0);
				}
			);
		});
	...
	describe('openOnFocus', function () {
		...
		it_n(
			"[enter] should not add item when dropdown isn't open",
			function (done) {
				const test = setup_test('AB_Multi', {
					openOnFocus: false,
				});
	...
	it_n('shadow root', async () => {
		setup_test(
			'<div><div id="shadow-wrapper"><select id="select-tags" multiple><option value="a">a</option><option value="b">b</option></div>'
		);

		function styleText(href) {
			const sheet = document.querySelector(`link[href*="${href}"]`).sheet;
