describe('Validation', function () {
	describe('<select required>', function () {
		let form, button, test;

		function test_required() {
			test = setup_test(
				`<form><select class="setup-here" required>
		...
		it_n('should have the required class', function () {
			test_required();
			expect(
				test.instance.wrapper.classList.contains('required')
			).to.be.equal(true);
		});
		...
		it_n(
			'should have "invalid" class when validation fails',
			function (done) {
				test_required();
				test.select.checkValidity();

				const classList = test.instance.wrapper.classList;

				assert.equal(
					classList.contains('invalid'),
					true,
					'invalid class missing'
				);
				...
				syn.click(button, function () {
					syn.click(test.instance.control, function () {
						assert.equal(
							classList.contains('invalid'),
							true,
							'invalid class missing after clicking control'
						);
						syn.click(document.body, function () {
							assert.equal(
								classList.contains('invalid'),
								true,
								'invalid class missing after clicking body'
							);
							done();
						});
					});
		...
		it_n(
			'should clear the invalid class after an item is selected',
			function (done) {
				test_required();
				syn.click(button, function () {
					assert.equal(
						test.instance.wrapper.classList.contains('invalid'),
						true
					);
					test.instance.addItem('a');
					assert.equal(
	...
	describe('<select> (not required)', function () {
		...
		it_n('should not have the required class', function () {
			expect(
				test.instance.control.classList.contains('required')
			).to.be.equal(false);
		});
	...
	describe('<input pattern>', function () {
		...
		it_n('should be valid if pattern matches', function () {
			const test = setup_test(
				'<input pattern="[a-z]+" value="AA" required />',
				{ create: true }
			);
