...
describe('Events', function () {
	describe('focus', function () {
		it_n('should work as expected', function (done) {
			const test = eventTest(
				'focus',
				'<select><option value="a" selected></option><option value="b"></option><option value="c"></option></select>',
				{}
			);
	...
	describe('blur', function () {
		it_n(
			'blur event should be called when clicking on body',
			function (done) {
				const test = eventTest(
					'blur',
					'<select id="blur-test"><option value="a" selected></option><option value="b"></option><option value="c"></option></select>',
					{}
				);
	...
	describe('change', function () {
		...
		it_n('should contain current value (multiple)', function (done) {
			const test = setup_test(
				'<select multiple><option value="a" selected></option><option value="b"></option><option value="c"></option></select>'
			);
			test.instance.on('change', function (value) {
		...
		it_n(
			'should not be triggered when the selected item has not changed',
			function (done) {
				const test = eventTest(
					'change',
					'<select><option value="a" selected="selected">a</option></select>'
				);

				syn.click(test.instance.control).delay(0, function () {
					syn.click(
						$('[data-value="a"]', $(test.instance.dropdown))
					).delay(0, function () {
		...
		it_n(
			'should not be possible to trigger a disabled option',
			function (done) {
				const test = eventTest(
					'change',
					`<select>
		...
		it_n(
			...
			function (done) {
				...
				syn.click(test.instance.control).delay(0, function () {
					syn.click(
						$('[data-value="c"]', $(test.instance.dropdown))
					).delay(0, function () {
						expect(test.counter).to.be.equal(0);
						done();
					});
	...
	describe('input', function () {
		it_n('should be triggered once before change', function (done) {
			...
			let evt = '';
			test.select.addEventListener('change', function () {
				evt = evt + 'change';
			});
			test.select.addEventListener('input', function () {
				evt = evt + 'input';
			});
			test.instance.setValue('b');
	...
	describe('item_add', function () {
		it_n("should contain item's value and element", function (done) {
			const test = setup_test(
				'<select><option value="a"></option><option value="b"></option><option value="c"></option></select>',
				{}
			);
	...
	describe('item_remove', function () {
		it_n("should contain item's value and element", function (done) {
			const test = setup_test(
				'<select multiple><option value="a" selected></option><option value="b" selected></option><option value="c"></option></select>',
				{}
			);
	...
	describe('type', function () {
		it_n('should contain current value', function (done) {
			const test = setup_test('<select></select>', { create: true });
			test.instance.on('type', function (value) {
				expect(value).to.be.equal('a');
				done();
			});
			syn.click(test.instance.control).type(
				'a',
