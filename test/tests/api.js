describe('API', function () {
	describe('disable()', function () {
		...
		it_n('should set "disabled" class', function () {
			expect(
				test.instance.wrapper.classList.contains('disabled')
			).to.be.equal(true);
		});
		...
		it_n('should add "disabled" attribute on inputs', function () {
			...
			expect($(test.instance.control_input).is(':disabled')).to.be.equal(
				true
			);
		});
	});
	...
	describe('enable()', function () {
		...
		it_n('should remove "disabled" class', function () {
			expect(
				test.instance.control.classList.contains('disabled')
			).to.be.equal(false);
		});
		...
		it_n('should focus and blur', function (done) {
			...
			window.setTimeout(function () {
				assert.isTrue(
					...
					'should set isFocused property to true'
				);
				assert.isTrue(
					has_focus(test.instance.control_input),
					'should give the control focus'
				);
				...
				window.setTimeout(() => {
					assert.isFalse(
						test.instance.isFocused,
						'should set isFocused property to false'
					);
					assert.isFalse(
						has_focus(test.instance.control_input),
						'should remove focus from the control'
					);
					done();
				}, 5);
			}, 5);
	...
	describe('createItem()', function () {
		it_n(
			'should fail if boolean returned by "create" callback',
			function () {
				const test = setup_test('<select>', {
					valueField: 'value',
					labelField: 'value',
		...
		it_n(
			'should fail if string returned by "create" callback',
			function () {
				const test = setup_test('<select>', {
					valueField: 'value',
					labelField: 'value',
				...
				var test = setup_test('<select>', {
					...
					create(input, callback) {
						window.setTimeout(function () {
							...
							expect(test.instance.options).to.have.property(
								'test'
							);
							done();
						}, 0);
	...
	describe('addOptionGroup()', function () {
		...
		before(function () {
			test = setup_test('<select>', {
				valueField: 'value',
				labelField: 'value',
				optgroupValueField: 'grpval',
			});
		});
		it_n('should register group', function () {
			const data = { label: 'Group Label' };
	...
	describe('addOption()', function () {
		...
		it_n('should add implicit $order property', function () {
			const opt1 = { value: 'hello' };
			const opt2 = { value: 'world' };
	...
	describe('addItem()', function () {
		...
		it_n('should not allow duplicate entries', function () {
			test.instance.addItem('a');
			test.instance.addItem('a');
			expect(test.instance.items.indexOf('a')).to.be.equal(
				test.instance.items.lastIndexOf('a')
			);
		});
		...
		it_n('should update DOM (1)', function () {
			...
			expect(
				$(test.instance.control).find('[data-value=c]').length
			).to.be.equal(1);
	...
	describe('addItems()', function () {
		...
		it_n('should add items in order', function (done) {
			...
			const selected = test.html.querySelectorAll('option:checked');
			const option_a = test.html.querySelector('option[value="a"]');
			const option_new1 = test.html.querySelector('option[value="new1"]');
			const option_b = test.html.querySelector('option[value="b"]');
			const option_new2 = test.html.querySelector('option[value="new2"]');

			assert.equal(
				selected.length,
				4,
				'should have four selected options'
			);
			assert.equal(
			...
			assert.equal(
				...
				option_new1,
				'"new1" should be after "a"'
			);
			assert.equal(
				option_new1.nextSibling,
				option_b,
				'"b" should be after "new1"'
			);
			assert.equal(
				option_b.nextSibling,
				option_new2,
				'"new2" should be after "b"'
			);
			done();
		});
	...
	describe('updateOption()', function () {
		...
		it_n('should update option data', function () {
			const option_el_before = test.instance.getOption('c');
			test.instance.setActiveOption(option_el_before);

			test.instance.updateOption('c', { value: 'c', test: 'test' });

			const option_el_after = test.instance.getOption('c');
			assert.notStrictEqual(
				option_el_before,
				option_el_after,
				'option DOM element should be new'
			);
			assert.isOk(
				option_el_after.classList.contains('active'),
				'new option el should be active'
			);
		...
		it_n('should maintain implicit $order property', function () {
			const order_orig = test.instance.options.x.$order;
		...
		it_n('should ignore undefined / null value references', function () {
			test.instance.updateOption(undefined, {
				value: 'undefined',
				test: 'test',
			});
			test.instance.updateOption(null, { value: 'null', test: 'test' });
			expect(test.instance.options.undefined).to.not.have.property(
				'test'
			);
		...
		it_n('should update item DOM (2)', function () {
			...
			assert.equal(
				test.instance.control.querySelectorAll('.item[data-value="f"]')
					.length,
				0
			);
			assert.equal(
				test.instance.control.querySelectorAll(
					'.item[data-value="f_updated"]'
				).length,
				1
			);
		});
		...
		it_n(
			'should not update options if attempting to update invalid option',
			function () {
				const opts_before = Object.assign({}, test.instance.options);
	...
	describe('getOption()', function () {
		...
		it_n('should return cached option', function () {
			...
			test.instance.refreshOptions(true);
			const option_before = test.instance.getOption('a');

			test.instance.control_input.value = 'b';
			test.instance.refreshOptions(true);
			const option_after = test.instance.getOption('a');
	...
	describe('clear()', function () {
		...
		it_n('should update DOM (3)', function () {
			...
			test.instance.clear();
			expect(
				test.instance.control.querySelectorAll('[data-value="1"]')
					.length
			).to.be.equal(0);
			expect(
				test.instance.control.querySelectorAll('[data-value="2"]')
					.length
			).to.be.equal(0);
			expect(
				test.instance.control.querySelectorAll('[data-value="3"]')
					.length
			).to.be.equal(0);
		});
		...
		it_n(
			'should create empty option and el.value should be empty after clear() on single',
			async function () {
				const test = setup_test('<select><option>a</option></select>');
				await asyncClick(test.instance.control);
				test.instance.clear();
				assert.isEmpty(test.select.value);
				assert.isOk(
					test.instance.input.querySelector('option[value=""]')
				);
			}
		);
		...
		it_n('should remove user created option', function () {
			...
			const len_opts_before = Object.keys(test.instance.options).length;
			test.instance.createItem('test');
	...
	describe('search()', function () {
		it_n(
			'should throw error if "score" setting does not return a function',
			function () {
				let test;
		...
		it_n(
			'should not throw error if "score" setting does return a function',
			function () {
				let test;
	...
	describe('getScoreFunction()', function () {
		it_n('should return an function that returns a number', function () {
			...
			const fn = test.instance.getScoreFunction('test');
	...
	describe('destroy()', function () {
		it_n('should remove control from DOM', function () {
			...
			assert.isFalse(
				document.documentElement.contains(test.instance.wrapper),
				'should remove control from DOM'
			);
			assert.isUndefined(
				test.instance.input.instance,
				'should delete "instance" reference on original input element'
			);
		});
		...
		it_n('should restore original options and tabindex', function () {
			const children =
				...
				'</optgroup>';
			const test = setup_test(
				'<select tabindex="9999">' + children + '</select>',
				{}
			);
			test.instance.destroy();
			expect(
				test.select.innerHTML,
				'restoring children failed'
			).to.be.equal(children);
			expect(
				test.select.tabIndex,
				'restoring tabindex failed'
			).to.be.equal(9999);
		});
	});
	...
	describe('clearCache()', function () {
		...
		it_n('should clear all cached option nodes', function () {
			const option_el_before = test.instance.getOption('0');

			assert.isOk(option_el_before);

			test.instance.clearCache();

			const option_el_after = test.instance.getOption('0');
			assert.isNull(
				option_el_after,
				'should clear option dom after clearCache()'
			);
		});

		it_n(
			'should return different item dom after clearCache()',
			function () {
				const first = test.instance.render(
					'item',
					test.instance.options[0]
				);
				test.instance.clearCache('item');
				const second = test.instance.render(
					'item',
					test.instance.options[0]
				);
	...
	describe('removeItem()', function () {
		...
		it_n('should remove item with value=0', function () {
			test = setup_test('<select multiple>', {
				...
				labelField: 'value',
				searchField: 'value',
				options: [{ value: 2 }, { value: 1 }, { value: 0 }],
				items: ['2', '1', '0'],
			});
			test.instance.refreshOptions(true);
	...
	describe('sync()', function () {
		it_n('sync() should update options', function () {
			...
			const opt_count = Object.keys(test.instance.options).length;
			const opt = new Option('new', 'new', true, true);
