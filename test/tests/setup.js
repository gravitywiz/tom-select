describe('Setup', function () {
	it_n('should not allow duplicate initialization', function () {
		const test = setup_test('<input type="text">', {});
		let errors = 0;
		try {
	...
	describe('<input type="text">', function () {
		...
		it_n(
			'should complete without exceptions - id with quotes',
			function () {
		...
		it_n(
			'should populate items,options from "dataAttr" if available',
			function () {
				const data = [
					{ val: 'a', lbl: 'Hello' },
					{ val: 'b', lbl: 'World' },
				...
				const test = setup_test(
					'<input type="text" value="c,d,e" data-hydrate="' +
		...
		it_n('should populate options from data attributes', function () {
			const test = setup_test(
				'<select><option value="a" data-test="b">c</option></select>'
			);
			const option =
				test.instance.input.querySelector('option[value="a"]');
		...
		it_n('should treat bool values as integers', function () {
			const test = setup_test('<input type="text">', {
				options: [
					{ value: true, label: 'Hello' },
					{ value: false, label: 'World' },
				],
			});
		...
		describe('getValue()', function () {
			it_n('should return value as a string', function () {
				const test = setup_test('<input type="text" value="a,b">', {
					delimiter: ',',
				});
				expect(test.instance.getValue()).to.be.a('string');
			});
			it_n('should return "" when empty', function () {
				const test = setup_test('<input type="text" value="">', {
					delimiter: ',',
				});
		...
		describe('<input type="text" attributes>', function () {
			it_n(
				'should propagate original input attributes to the generated input',
				function () {
					const test = setup_test(
						'<input type="text" autocorrect="off" autocapitalize="none" autocomplete="new-password">',
						{}
					);
					expect(
						test.instance.control_input.getAttribute('autocorrect')
					).to.be.equal('off');
					expect(
						test.instance.control_input.getAttribute(
							'autocapitalize'
						)
					).to.be.equal('none');
					expect(
						test.instance.control_input.getAttribute('autocomplete')
					).to.be.equal('new-password');
				}
			);
			it_n(
				'should not add attributes if not present in the original',
				function () {
					const test = setup_test('<input type="text">', {});
					expect(
						test.instance.control_input.getAttribute('autocorrect')
	...
	describe('<input type="number">', function () {
		it_n('should complete without exceptions', function (done) {
			const test = setup_test('<input type="number">', {});

			assert.equal(
				test.instance.control_input.getAttribute('type'),
				'number'
			);
			done();
		});
	...
	describe('<select>', function () {
		...
		it_n(
			...
			function () {
				...
				assert.equal(test.instance.options.a.value, 'a');
				assert.deepEqual(test.instance.options.a.grp, [
					'Group 1',
					'Group 2',
				]);
				assert.equal(test.instance.options.a.$order, 1);
				...
				assert.deepEqual(test.instance.options.b.grp, [
					'Group 1',
					'Group 2',
				]);
				assert.equal(test.instance.options.b.$order, 2);
		...
		it_n(
			'should respect disabled flags of option and optgroup',
			function () {
				const test = setup_test(
					[
						'<select>',
		...
		it_n(
			'should render optgroups with duplicated options correctly',
			function (done) {
				const test = setup_test(
					[
						'<select>',
				...
				assert.equal(
					test.instance.dropdown_content.querySelectorAll('.optgroup')
						.length,
					2,
					'expect 2 optgroups'
				);
		...
		it_n(
			'display non-optgroup items and optgroups with lockOptgroupOrder = true',
			function (done) {
				const test = setup_test(
					`<select>
					<option>Item</option>
		...
		it_n('should add options in text form (no html entities)', function () {
			const test = setup_test(
				'<select><option selected value="a">&lt;hi&gt;</option></select>',
				{}
			);
		...
		it_n(
			'should keep options in original order if no sort given',
			function (done) {
				const test = setup_test(
					[
						'<select multiple>',
		...
		it_n('should respect option disabled flag', function (done) {
			...
			expect(
				$(test.instance.dropdown).find('[data-selectable]')
			).to.has.length(1);
			done();
		});
		describe('getValue()', function () {
			...
			it_n('should return proper value when not empty', function () {
				const test = setup_test(
					'<select><option selected value="a">A</option></select>',
					{}
				);
	...
	describe('<select multiple>', function () {
		...
		describe('getValue()', function () {
			...
			it_n(
				'should return proper value as array when not empty',
				function () {
					const test = setup_test(
						'<select multiple><option selected value="a">A</option></select>',
						{}
					);
	...
	describe('<select> (custom string render)', function () {
		it_n('should render the custom option element', function (done) {
			...
			window.setTimeout(function () {
				assert.equal(
					test.instance.dropdown.querySelectorAll('.custom-option')
						.length,
					2
				);
				done();
	...
	describe('<select> (custom dom render)', function () {
		...
		it_n('should render the custom option element', function (done) {
			...
			window.setTimeout(function () {
				expect(
					$(test.instance.dropdown_content).find('.custom-option')
						.length
				).to.be.equal(1);
				done();
			}, 5);
	...
	describe('rtl detection', function () {
		...
		it_n('should detect rtl', function () {
			const test = setup_test('<select dir="rtl">', {});
			expect(test.instance.rtl).to.be.equal(true);
		});

		it_n('should detect parent rtl', function () {
			const test = setup_test(
				'<div dir="rtl"><select class="setup-here"></select></div>',
				{}
			);
			expect(test.instance.rtl).to.be.equal(true);
		});
	...
	describe('external control input', function () {
		...
		before(() => {
			test = setup_test(
				'<div><select class="setup-here"><option>a</option><option>b</option></select><input id="external-control"></div>',
				{ controlInput: '#external-control' }
			);
			control = document.getElementById('external-control');
		});

		it_n(
			'should filter options when typing in external control',
			function (done) {
		...
		it_n('should not move caret position', function (done) {
			...
			const caretpos = test.instance.caretPos;
